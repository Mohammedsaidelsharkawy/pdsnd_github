...
def city_input():
    ...
    while True:
            if city == 'chicago':
                ...
                print('\n Please enter the name of city correctly (Chicago, New york or washington)\n')
                city = input('Which city you would like to get : ')
                city = city.lower()
    return city

def get_time():
    period = input('\n You want to filter the data by month and day of the month, day of the week, or not at all? Type "no" for no time filter.\n')
    period = period.lower()
    ...
    while True: 
        if period == "month":
            while True:
                ...
                if day_month == "no":
                    print('\n collecting statistics about month...\n')
                    return 'month'
                elif day_month == "yes":
                   print ('\n collecting statistics about day of month...\n')
                   return 'day_of_month'
                
        if period == "day":
            print('\n collecting data about the day of the week...\n')
            return 'day_of_week'
        elif period == "no":
            print('\n No time filter will be done\n')
            return "none"
        period = input("\n Please choose a time filter either 'month', 'day' of the week, or none (no) \n").lower()

def month_info(m):
    if m == 'month':
        month = input('\nChoose month: January, February, March, April, May, or June? \n')
        while month.strip().lower() not in ['january', 'february', 'march', 'april', 'may', 'june']:
            month = input('\nPlease choose either January, February, March, April, May, or June only... Please type the full month name.\n')
...
def main():
    ...
    restart = input("\n * Would you like to perform other statistics? Type \'yes\' or \'no\'.\n")
